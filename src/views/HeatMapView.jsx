...
import { FROST_REGIONS, RISK_COLORS, REGIONS } from '../data/frostData'
  ...
  useEffect(() => {
    ...
    if (visible) {
      heatRef.current = window.L.heatLayer(points, {
        ...
        gradient: { 0.0: '#0d47a1', 0.3: '#1565c0', 0.5: '#ff8c42', 0.7: '#ef5350', 1.0: '#b71c1c' },
      }).addTo(map)
    }
...
export default function HeatMapView() {
  ...
  useEffect(() => {
    if (window.L && window.L.heatLayer) { setHeatLoaded(true); return }
  ...
  const heatPoints = filtered.flatMap(region => {
    const pts = []
    for (let i = 0; i < 8; i++) {
      pts.push([region.lat + (Math.random() - 0.5) * 3, region.lng + (Math.random() - 0.5) * 3, region.intensity])
    }
    pts.push([region.lat, region.lng, region.intensity])
    return pts
  })
      ...
      <div className="view-header">
        ...
        <div className="header-controls">
          <div className="layer-toggles">
            <button className={`layer-btn ${showHeat ? 'active' : ''}`} onClick={() => setShowHeat(!showHeat)}>
              <span className="layer-dot heat-dot"></span>Heat Layer
            </button>
            <button className={`layer-btn ${showMarkers ? 'active' : ''}`} onClick={() => setShowMarkers(!showMarkers)}>
              <span className="layer-dot marker-dot"></span>Regions
            </button>
          </div>
          <select className="region-select" value={selectedRegion} onChange={e => setSelectedRegion(e.target.value)}>
            {REGIONS.map(r => <option key={r} value={r}>{r}</option>)}
          </select>
          <select className="region-select" value={filterRisk} onChange={e => setFilterRisk(e.target.value)}>
      ...
      <div className="map-wrapper">
        <MapContainer center={[30, 10]} zoom={2} style={{ width: '100%', height: '100%' }} minZoom={2} maxZoom={10}>
          ...
          {showMarkers && filtered.map(region => {
            ...
            return (
              <CircleMarker key={region.id} center={[region.lat, region.lng]}
              ...
              >
                <Popup>
                  <div className="popup-content">
                    ...
                    <div className="popup-row"><span>Risk level</span><span style={{ color: RISK_COLORS[region.cropRisk]?.color }}>{RISK_COLORS[region.cropRisk]?.label}</span></div>
                    <div className="popup-row"><span>Intensity</span><span>{Math.round(region.intensity * 100)}%</span></div>
                    {region.economicLoss && <div className="popup-row"><span>Economic loss</span><span>${(region.economicLoss / 1e9).toFixed(1)}B</span></div>}
                    {region.adoptionOutlook && <div className="popup-row"><span>Outlook</span><span style={{ textTransform: 'capitalize' }}>{region.adoptionOutlook}</span></div>}
                    <div className="popup-crops">{region.crops.map(c => <span key={c} className="popup-crop-tag">{c}</span>)}</div>
        ...
        <div className="map-stats">
          <div className="map-stat"><span className="map-stat-value">{filtered.length}</span><span className="map-stat-label">Regions shown</span></div>
          <div className="map-stat-sep"></div>
          <div className="map-stat"><span className="map-stat-value" style={{ color: '#ef5350' }}>{riskStats['very-high']}</span><span className="map-stat-label">Very high risk</span></div>
          <div className="map-stat-sep"></div>
          <div className="map-stat"><span className="map-stat-value" style={{ color: '#ff8c42' }}>{riskStats['high']}</span><span className="map-stat-label">High risk</span></div>
        </div>
      </div>
    </div>
  )
