...
const KEY_FACTS = [
  { label: 'Regions Mapped', value: FROST_REGIONS.length, sub: 'across 6 continents', accent: '#4fc3f7' },
  { label: 'Countries Covered', value: [...new Set(FROST_REGIONS.map(r => r.country))].length, sub: 'priority markets identified', accent: '#4fc3f7' },
  { label: 'Very High Risk Regions', value: FROST_REGIONS.filter(r => r.cropRisk === 'very-high').length, sub: 'requiring immediate focus', accent: '#ef5350' },
  { label: 'Crop Types Identified', value: [...new Set(FROST_REGIONS.flatMap(r => r.crops))].length, sub: 'unique varieties tracked', accent: '#ff8c42' },
]
      ...
      <div className="overview-grid">
            ...
            {PROJECT_ITEMS.map(item => (
              <div key={item.id} className="tracker-row">
                ...
                <div className="tracker-right">
                  <span className="tracker-badge" style={{ color: PRIORITY_CONFIG[item.priority].color, background: PRIORITY_CONFIG[item.priority].color + '18' }}>
                    {PRIORITY_CONFIG[item.priority].label}
                  </span>
                  <span className="tracker-badge" style={{ color: STATUS_CONFIG[item.status].color, background: STATUS_CONFIG[item.status].color + '18' }}>
        ...
        <div className="card context-card">
          ...
          <div className="context-rows">
            <div className="context-item"><span className="context-label">Protection range</span><span className="context-val accent-cold">Up to −8°C</span></div>
            <div className="context-item"><span className="context-label">vs. incumbents</span><span className="context-val accent-muted">~−2°C (wind machines, burners)</span></div>
            <div className="context-item"><span className="context-label">Near-term cost</span><span className="context-val">~$50/acre</span></div>
            <div className="context-item"><span className="context-label">Target sell price</span><span className="context-val accent-warm">~$20/acre</span></div>
            <div className="context-item"><span className="context-label">Long-term scaled cost</span><span className="context-val accent-success">~$5/acre</span></div>
            <div className="context-item"><span className="context-label">US grower interviews</span><span className="context-val">100+ completed</span></div>
          </div>
